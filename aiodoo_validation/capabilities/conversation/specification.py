"""Conversation CapabilitySpecification instance (mirrors capability.yaml)."""
...
CONVERSATION_CAPABILITY_ID = "conversation"
CONVERSATION_PARSER_ID = "conversation_v1"
CONVERSATION_CORPUS_SCHEMA_ID = "conversation_tasks_v1"


def capability_yaml_path() -> Path:
    """Path to the declarative Conversation capability.yaml."""
    return Path(__file__).resolve().parent / CAPABILITY_YAML_NAME


def build_conversation_specification() -> CapabilitySpecification:
    """
    Construct the frozen Conversation Capability Specification.
    ...
    return CapabilitySpecification(
        id=CONVERSATION_CAPABILITY_ID,
        name="Conversation",
        description="Independent conversation capability adapter validation",
        version="1.0.0",
        ...
        corpus_schema_id=CONVERSATION_CORPUS_SCHEMA_ID,
