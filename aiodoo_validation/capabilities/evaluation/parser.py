"""EvaluationRecordParser — native Evaluation JSON → ParsedCapabilityRecord."""
...
def _require_mapping(value: Any, *, label: str) -> Mapping[str, Any]:
    if not isinstance(value, Mapping):
        raise EvaluationParseError(f"{label} must be a JSON object.")
    return value


def _require_str(data: Mapping[str, Any], key: str, *, label: str) -> str:
    if key not in data:
        raise EvaluationParseError(f"{label} missing required field {key!r}.")
    value = data[key]
    if not isinstance(value, str):
        raise EvaluationParseError(f"{label} field {key!r} must be a string.")
    return value
...
def _optional_str(data: Mapping[str, Any], key: str, *, label: str) -> str | None:
    ...
    value = data[key]
    if not isinstance(value, str):
        raise EvaluationParseError(f"{label} field {key!r} must be a string when set.")
    return value
        ...
        if key in data and data[key] is not None:
            ...
            if not isinstance(value, str) or value.strip() not in _SUPPORTED_SCHEMA_IDS:
                raise EvaluationParseError(
                    f"Unsupported Evaluation schema_id {value!r}; "
    ...
    if isinstance(metadata, Mapping):
        for key in ("schema_id", "schema_version", "corpus_schema_id"):
            if key in metadata and metadata[key] is not None:
                ...
                if not isinstance(value, str) or value.strip() not in _SUPPORTED_SCHEMA_IDS:
                    raise EvaluationParseError(
                        f"Unsupported Evaluation metadata[{key!r}]={value!r}; "
                        f"allowed: {sorted(_SUPPORTED_SCHEMA_IDS)}."
                    )


@dataclass(frozen=True, slots=True)
class EvaluationRecordParser:
    """
    Convert Evaluation dataset JSON into generic ``ParsedCapabilityRecord`` values.

    Accepts:
    - A native Evaluation **task** object (``id`` + ``problem`` + ``context`` + …)
    ...
    def parse_training_example(
        ...
        payload = _require_mapping(data, label="Evaluation training example")
        _check_schema_id(payload)
        if not self._is_training_envelope(payload):
            raise EvaluationParseError(
                "Training example must include instruction and output.tasks."
            )
        instruction = _require_str(payload, "instruction", label="Evaluation training example")
        output = _require_mapping(payload.get("output"), label="output")
        tasks = output.get("tasks")
        if not isinstance(tasks, list) or not tasks:
            raise EvaluationParseError("output.tasks must be a non-empty list.")
    ...
    def _parse_compact(self, data: Mapping[str, Any]) -> ParsedCapabilityRecord:
        record_id = _require_str(data, "record_id", label="compact Evaluation record")
        problem = data.get("problem", "")
        if not isinstance(problem, str):
            raise EvaluationParseError("compact record field 'problem' must be a string.")
        artifacts_raw = data.get("artifacts")
        if not isinstance(artifacts_raw, list):
            raise EvaluationParseError("compact record field 'artifacts' must be a list.")
        artifacts = tuple(
        ...
        if not isinstance(operations, list):
            raise EvaluationParseError("compact record field 'operations' must be a list.")
        transforms = self._operations_to_descriptors(
            operations,
            artifacts=artifacts,
            label="compact operations",
        )
        explanation = _optional_str(data, "explanation", label="compact Evaluation record")
    ...
    def _parse_task(
        ...
        *,
        ...
        record_id = _require_str(task, "id", label="Evaluation task")
        ...
        if not isinstance(context_raw, list):
            raise EvaluationParseError("Evaluation task field 'context' must be a list.")
        if not context_raw:
            raise EvaluationParseError(
                f"Evaluation task {record_id!r} requires at least one context artifact."
            )
        ...
        if not isinstance(expected_raw, list):
            raise EvaluationParseError("expected_outcome.artifacts must be a list.")
        ...
        if operations is None:
            operations = []
        if not isinstance(operations, list):
            raise EvaluationParseError("expected_outcome.operations must be a list.")
        if not expected_artifacts and not operations:
            raise EvaluationParseError(
                f"Evaluation task {record_id!r} expected_outcome requires "
    ...
    def _artifact_from_mapping(
        ...
        *,
        ...
        if "content" not in data:
            raise EvaluationParseError(f"{source}[{index}] missing required field 'content'.")
        content = data["content"]
        if not isinstance(content, str):
            raise EvaluationParseError(f"{source}[{index}] field 'content' must be a string.")
        ...
        try:
            ...
            raise EvaluationParseError(str(exc)) from exc
        ...
        *,
        ...
        for index, raw in enumerate(operations):
            ...
            if "search" not in op or "replace" not in op:
                raise EvaluationParseError(f"{label}[{index}] must include 'search' and 'replace'.")
            search = op["search"]
            replace = op["replace"]
            ...
            if search == "":
                raise EvaluationParseError(f"{label}[{index}] field 'search' must be non-empty.")
            path = op.get("path")
            if path is None:
    ...
    def _parse_tags(raw: Any) -> tuple[str, ...]:
        ...
        if not isinstance(raw, list):
            raise EvaluationParseError("tags must be a list of strings when set.")
    ...
    def _merge_metadata(
        ...
        *,
        ...
        if raw is None:
            ...
            raise EvaluationParseError("metadata must be a JSON object when set.")
...
__all__ = ["EvaluationRecordParser"]
