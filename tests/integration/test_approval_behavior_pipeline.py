"""Integration tests for Approval behavioral production wiring (Phase 2)."""
...
class _ScriptedInferenceRunner:
    """Deterministic inference for approval behavior integration tests."""
...
def _bundle() -> ArtifactBundle:
    return ArtifactBundle(
        ...
        adapter=ArtifactDescriptor(
            ...
            logical_ref="adapter",
            location_digest="y",
            metadata={"adapter_type": "approval"},
        ),
        merged_model=None,
        protocol_major=1,
        protocol_minor=0,
...
class TestApprovalCorpusProvider:
...
class TestApprovalCapabilityBehaviorPipeline:
    def test_assemble_eval_corpus(self) -> None:
        loaded = ConfigurableCorpusProvider().load(EVAL_CORPUS)
        assert loaded is not None
        pack = create_default_capability_registry().get("approval")
...
class TestApprovalBehaviorOracle:
    def test_missing_corpus_defers(self) -> None:
        oracle = build_capability_behavioral_oracle(
            capability_id="approval",
            oracle_id=approval_behavior_oracle_id(),
            name="Approval Behavior",
            description="test",
    ...
    def test_successful_end_to_end(self) -> None:
        # Scripted responses are now the canonical ``ApprovalResponse`` JSON
            ...
            "Rename the reply template label.": json.dumps(
                {
                    ...
                    "status": "approved",
                    "reason": "Update reply template label.",
                }
            ),
            "Add approval turn helper.": json.dumps(
                {
        ...
        assert result.metadata["capability_id"] == "approval"
        ...
        oracle = build_capability_behavioral_oracle(
            ...
            inference_runner=_ScriptedInferenceRunner(
                texts={
                    ...
                    "Add approval turn helper.": "also wrong",
                }
...
class TestApprovalCorpusPin:
    def test_builtin_pin_resolves_approval_fixture(self) -> None:
        registry = builtin_corpus_pin_registry()
        pin = registry.get(APPROVAL_EVAL_FIXTURE_CORPUS_ID)
        assert pin.capability_id == "approval"
        assert registry.get("approval.eval").corpus_id == APPROVAL_EVAL_FIXTURE_CORPUS_ID


class TestApprovalPlanCorpusResolution:
    def test_plan_includes_resolved_corpus_path(self) -> None:
        ...
        request = ValidationRequest(
            ...
            adapter_ref="adapter",
            execution_tier=ExecutionTier.SMOKE,
            metadata={EVALUATION_CORPUS_ID_KEY: "approval.eval"},
        )
        ...
        assert plan.configuration.get(EVALUATION_CORPUS_ID_KEY) == APPROVAL_EVAL_FIXTURE_CORPUS_ID
        assert EVALUATION_CORPUS_PATH_KEY in plan.configuration
        oracle_ids = {s.stage_id for s in plan.oracle_pipeline}
        assert "approval.oracle.behavior.approval" in oracle_ids


class TestApprovalScoreCertReportFactories:
    def test_create_for_approval_factories(self) -> None:
        score = BehavioralEvidenceScorePolicy.create_for_approval()
        assert score.metadata.policy_id == APPROVAL_SCORE_BEHAVIOR
        assert score.metadata.supported_profile == "approval"
        cert = BehaviorGatedCertificationPolicy.create_for_approval()
        assert cert.metadata.policy_id == APPROVAL_CERTIFICATION_BEHAVIOR
        assert cert.metadata.supported_profile == "approval"
        assert cert.source_score_policy_id == APPROVAL_SCORE_BEHAVIOR
        reports = default_production_report_templates(profile="approval")
        assert any(t.metadata.template_id == "approval.report.behavior" for t in reports)


class TestProductionApprovalRegistration:
    def test_production_registers_approval_behavior_oracle(self) -> None:
    ...
    def test_production_does_not_import_approval_parser_directly(self) -> None:
        import inspect

        import aiodoo_validation.production as production_mod

        source = inspect.getsource(production_mod)
        assert "ApprovalRecordParser" not in source
