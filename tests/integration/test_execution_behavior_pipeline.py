"""Integration tests for Execution behavioral production wiring (Phase 2)."""
...
class _ScriptedInferenceRunner:
    """Deterministic inference for execution behavior integration tests."""
...
def _bundle() -> ArtifactBundle:
    return ArtifactBundle(
        ...
        adapter=ArtifactDescriptor(
            ...
            logical_ref="adapter",
            location_digest="y",
            metadata={"adapter_type": "execution"},
        ),
        merged_model=None,
        protocol_major=1,
        protocol_minor=0,
...
class TestExecutionCorpusProvider:
...
class TestExecutionCapabilityBehaviorPipeline:
    def test_assemble_eval_corpus(self) -> None:
        loaded = ConfigurableCorpusProvider().load(EVAL_CORPUS)
        assert loaded is not None
        pack = create_default_capability_registry().get("execution")
...
class TestExecutionBehaviorOracle:
    ...
    def test_successful_end_to_end(self) -> None:
        # Scripted responses are now the canonical ``ExecutionResponse`` JSON
            ...
            "Rename the reply template label.": json.dumps(
                {
                    ...
                    "status": "succeeded",
                    "stdout": "Update reply template label.",
                }
            ),
            "Add execution turn helper.": json.dumps(
                {
        ...
        assert result.metadata["capability_id"] == "execution"
        ...
        oracle = build_capability_behavioral_oracle(
            ...
            inference_runner=_ScriptedInferenceRunner(
                texts={
                    ...
                    "Add execution turn helper.": "also wrong",
                }
...
class TestExecutionCorpusPin:
    def test_builtin_pin_resolves_execution_fixture(self) -> None:
        registry = builtin_corpus_pin_registry()
        pin = registry.get(EXECUTION_EVAL_FIXTURE_CORPUS_ID)
        assert pin.capability_id == "execution"
        assert registry.get("execution.eval").corpus_id == EXECUTION_EVAL_FIXTURE_CORPUS_ID


class TestExecutionPlanCorpusResolution:
    def test_plan_includes_resolved_corpus_path(self) -> None:
        ...
        request = ValidationRequest(
            ...
            adapter_ref="adapter",
            execution_tier=ExecutionTier.SMOKE,
            metadata={EVALUATION_CORPUS_ID_KEY: "execution.eval"},
        )
        ...
        assert plan.configuration.get(EVALUATION_CORPUS_ID_KEY) == EXECUTION_EVAL_FIXTURE_CORPUS_ID
        assert EVALUATION_CORPUS_PATH_KEY in plan.configuration
        oracle_ids = {s.stage_id for s in plan.oracle_pipeline}
        assert "execution.oracle.behavior.execution" in oracle_ids


class TestExecutionScoreCertReportFactories:
    def test_create_for_execution_factories(self) -> None:
        score = BehavioralEvidenceScorePolicy.create_for_execution()
        assert score.metadata.policy_id == EXECUTION_SCORE_BEHAVIOR
        assert score.metadata.supported_profile == "execution"
        cert = BehaviorGatedCertificationPolicy.create_for_execution()
        assert cert.metadata.policy_id == EXECUTION_CERTIFICATION_BEHAVIOR
        assert cert.metadata.supported_profile == "execution"
        assert cert.source_score_policy_id == EXECUTION_SCORE_BEHAVIOR
        reports = default_production_report_templates(profile="execution")
        assert any(t.metadata.template_id == "execution.report.behavior" for t in reports)


class TestProductionExecutionRegistration:
    def test_production_registers_execution_behavior_oracle(self) -> None:
    ...
    def test_production_does_not_import_execution_parser_directly(self) -> None:
        ...
        assert "ExecutionRecordParser" not in source
