"""Integration tests for Conversation behavioral production wiring (Phase 2)."""
...
class _ScriptedInferenceRunner:
    """Deterministic inference for conversation behavior integration tests."""
...
def _bundle() -> ArtifactBundle:
    return ArtifactBundle(
        ...
        adapter=ArtifactDescriptor(
            ...
            logical_ref="adapter",
            location_digest="y",
            metadata={"adapter_type": "conversation"},
        ),
        merged_model=None,
        protocol_major=1,
        protocol_minor=0,
...
class TestConversationCorpusProvider:
...
class TestConversationCapabilityBehaviorPipeline:
    def test_assemble_eval_corpus(self) -> None:
        loaded = ConfigurableCorpusProvider().load(EVAL_CORPUS)
        assert loaded is not None
        pack = create_default_capability_registry().get("conversation")
...
class TestConversationBehaviorOracle:
    def test_missing_corpus_defers(self) -> None:
        oracle = build_capability_behavioral_oracle(
            capability_id="conversation",
            oracle_id=conversation_behavior_oracle_id(),
            name="Conversation Behavior",
            description="test",
    ...
    def test_successful_end_to_end(self) -> None:
        ...
        assert result.metadata["capability_id"] == "conversation"
        ...
        oracle = build_capability_behavioral_oracle(
            ...
            inference_runner=_ScriptedInferenceRunner(
                texts={
                    ...
                    "Add conversation turn helper.": "also wrong",
                }
...
class TestConversationCorpusPin:
    def test_builtin_pin_resolves_conversation_fixture(self) -> None:
        registry = builtin_corpus_pin_registry()
        pin = registry.get(CONVERSATION_EVAL_FIXTURE_CORPUS_ID)
        assert pin.capability_id == "conversation"
        assert registry.get("conversation.eval").corpus_id == CONVERSATION_EVAL_FIXTURE_CORPUS_ID


class TestConversationPlanCorpusResolution:
    def test_plan_includes_resolved_corpus_path(self) -> None:
        ...
        request = ValidationRequest(
            ...
            adapter_ref="adapter",
            execution_tier=ExecutionTier.SMOKE,
            metadata={EVALUATION_CORPUS_ID_KEY: "conversation.eval"},
        )
        ...
        assert (
            plan.configuration.get(EVALUATION_CORPUS_ID_KEY) == CONVERSATION_EVAL_FIXTURE_CORPUS_ID
        )
        assert EVALUATION_CORPUS_PATH_KEY in plan.configuration
        oracle_ids = {s.stage_id for s in plan.oracle_pipeline}
        assert "conversation.oracle.behavior.conversation" in oracle_ids


class TestConversationScoreCertReportFactories:
    def test_create_for_conversation_factories(self) -> None:
        score = BehavioralEvidenceScorePolicy.create_for_conversation()
        assert score.metadata.policy_id == CONVERSATION_SCORE_BEHAVIOR
        assert score.metadata.supported_profile == "conversation"
        cert = BehaviorGatedCertificationPolicy.create_for_conversation()
        assert cert.metadata.policy_id == CONVERSATION_CERTIFICATION_BEHAVIOR
        assert cert.metadata.supported_profile == "conversation"
        assert cert.source_score_policy_id == CONVERSATION_SCORE_BEHAVIOR
        reports = default_production_report_templates(profile="conversation")
        assert any(t.metadata.template_id == "conversation.report.behavior" for t in reports)


class TestProductionConversationRegistration:
    def test_production_registers_conversation_behavior_oracle(self) -> None:
    ...
    def test_production_does_not_import_conversation_parser_directly(self) -> None:
        ...
        assert "ConversationRecordParser" not in source
